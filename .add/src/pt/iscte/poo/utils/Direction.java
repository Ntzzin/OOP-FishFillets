...
public enum Direction implements Serializable {
	LEFT(new Vector2D(-1,0)), UP(new Vector2D(0,-1)), RIGHT(new Vector2D(1,0)), DOWN(new Vector2D(0,1)), NONE(new Vector2D(0,0));
	// Adicionado NONE para linha 37.
	...
	public static Direction directionFor(int keyCode) {
		switch(keyCode){
			...
			case KeyEvent.VK_RIGHT:
				return RIGHT;
			default: // Adicionado um caso default para não crashar oque induz a:
				return NONE;
		}
		// Tirado execption
	}
